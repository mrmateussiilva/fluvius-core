...
<template>
  <aside class="absolute inset-0 z-30 flex h-full w-full shrink-0 flex-col border-l border-line bg-canvas xl:static xl:w-[340px]">
    <header class="flex min-h-[64px] items-center justify-between border-b border-line bg-panel-muted px-4 py-3">
    ...
    <div class="soft-scrollbar min-h-0 flex-1 overflow-y-auto">
      <div class="bg-panel px-5 py-6 text-center shadow-sm">
        <img
          v-if="contact?.profile_picture_url && !imageFailed"
          :src="contact.profile_picture_url"
          :alt="displayName"
          class="mx-auto h-24 w-24 rounded-full object-cover ring-4 ring-line"
        ...
        <h3 class="mt-3 text-lg font-semibold text-ink">{{ displayName }}</h3>
        ...
        <div
          ...
          class="mt-2 inline-flex items-center gap-1 rounded-full bg-success-soft px-2 py-1 text-xs font-medium text-success-strong"
        >
          <CheckCircle2 class="h-3.5 w-3.5" /> Número no WhatsApp
        </div>
        <div
          v-else-if="contact?.is_on_whatsapp === false"
          class="mt-2 inline-flex rounded-full bg-danger-soft px-2 py-1 text-xs font-medium text-danger-strong"
        >
          Número não encontrado no WhatsApp
        </div>
      </div>

      <div v-if="loading && !contact" class="m-4 rounded-lg bg-panel py-10 text-center text-sm text-ink-muted shadow-sm">
        Consultando dados do WhatsApp...
      </div>
      <p v-if="error" class="m-4 rounded-lg bg-danger-soft p-3 text-xs text-danger-strong ring-1 ring-danger/20">
        {{ error }}
      </p>

      <template v-if="contact">
        <div v-if="contact.verified_name || contact.business_name" class="mx-4 mt-4 rounded-lg bg-panel p-4 shadow-sm">
          <div class="flex items-center gap-2 text-sm font-medium text-ink-secondary">
            <BadgeCheck class="h-4 w-4 text-success" /> Perfil comercial
          </div>
        ...
        <div v-if="contact.about" class="mx-4 mt-3 rounded-lg bg-panel p-4 shadow-sm">
          <p class="text-[10px] font-medium uppercase tracking-[0.12em] text-ink-faint">Recado</p>
          <p class="mt-1.5 text-sm text-ink">{{ contact.about }}</p>
        </div>

        <div class="mx-4 mt-3 grid grid-cols-2 gap-2">
          <div class="rounded-lg bg-panel p-3 shadow-sm">
        ...
        <dl class="mx-4 mt-3 space-y-4 rounded-lg bg-panel p-4 text-sm shadow-sm">
          <div>
            <dt class="flex items-center gap-1.5 text-xs font-medium uppercase tracking-wide text-ink-faint">
              <CalendarDays class="h-3.5 w-3.5" /> Primeira interação
            </dt>
            <dd class="mt-1 text-ink-secondary">{{ formatDate(contact.first_interaction_at) }}</dd>
          </div>
          <div>
            <dt class="flex items-center gap-1.5 text-xs font-medium uppercase tracking-wide text-ink-faint">
              <CalendarDays class="h-3.5 w-3.5" /> Última interação
            </dt>
            <dd class="mt-1 text-ink-secondary">{{ formatDate(contact.last_interaction_at) }}</dd>
          </div>
        </dl>

        <div
          v-if="(contact?.kind || conversation.contact_kind || 'direct') === 'group' && contact?.group_member_count !== null"
          class="mx-4 mt-3 rounded-lg bg-panel p-3 shadow-sm"
        >
          <div class="flex items-center gap-1.5 text-xs text-ink-muted">
        ...
        <div
          ...
          class="mx-4 mt-3 rounded-lg bg-panel p-3 shadow-sm max-h-48 overflow-y-auto"
        >
          <div class="flex items-center gap-1.5 text-xs font-medium uppercase tracking-wide text-ink-faint mb-2">
          ...
          <ul class="space-y-1">
            <li
              ...
              class="flex items-center gap-2 text-sm text-ink-secondary py-1"
            >
              <span
        ...
        <button
          class="mx-4 mt-3 flex items-center justify-center gap-2 rounded-lg border border-line bg-panel px-3 py-2.5 text-sm font-medium text-fluvius-700 shadow-sm transition hover:bg-fluvius-50 disabled:opacity-50"
          :disabled="loading"
          @click="emit('refresh')"
        >
        ...
        <p class="px-5 pb-5 pt-3 text-center text-[11px] leading-4 text-ink-faint">
          {{
            (contact?.kind || conversation.contact_kind || 'direct') === 'group'
