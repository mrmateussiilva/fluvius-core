...
| `ChannelTypebotConfig` | Habilitação e public ID do Typebot por canal | `tenant_id` |
| `BotSession` | Ciclo Typebot e IDs externos, sem estrutura do fluxo | `tenant_id` |
| `BotTurn` | Incoming e estado durável de cada tentativa Typebot | `tenant_id` |
...
`20260906_0029_typebot.py` adiciona as três tabelas Typebot, sem alterar
`channel_ai_configs`. Configuração é única por tenant/canal; o índice parcial
`uq_bot_session_active` permite no máximo uma sessão `active` por tenant/conversa.
Sessões usam `active`, `ended` e `failed`; `external_session_id` permanece nulo
durante a reserva inicial, antes do retorno de `startChat`. `public_id` é um
snapshot do fluxo escolhido para esse ciclo. Turnos são únicos por
tenant/mensagem e usam `queued`, `processing`, `completed`, `discarded` e
`failed`. O claim `processing` é persistido antes do POST; um claim interrompido
nunca é repetido automaticamente. Os enums de atendimento/mensagem permanecem
inalterados.
