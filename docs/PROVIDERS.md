...
Mídia usa `/send/media` com `type` igual a `image`, `audio`, `video` ou `document`; GIF sai como `document` porque o gateway só aceita JPEG/PNG/WebP como `image`. Figurinhas são normalizadas como WebP 512×512 pelo composer, persistidas com `message_type=sticker`, enviadas sem legenda por `/send/sticker` no campo `sticker` e renderizadas sem o balão de mídia comum. WebP recebido do WhatsApp segue o mesmo tipo nativo e pode ser animado. A URL persistida (origem pública) é convertida para o base interno de `EVOLUTION_GO_MEDIA_BASE_URL` (padrão `http://api:8000`) antes da chamada ao gateway, porque o Caddy de produção responde 404 para `/storage/*` na origem pública e o gateway não checa o status do download. `caption` e `filename` só entram no payload quando preenchidos — o gateway faz bind desses campos como string e rejeita `null`. Respostas citadas também são encaminhadas nos dois endpoints, e o envio de mídia usa timeout estendido de 60 s para cobrir download mais upload no WhatsApp. Erros HTTP do gateway incluem um trecho sanitizado e limitado do corpo de erro (por exemplo, `Invalid file format`), sem credenciais. O UUID criado pelo navegador acompanha o multipart como `client_message_id`, nasce como ID local `pending` e é reutilizado como chave idempotente no provider; o SHA-256 do conteúdo impede que o mesmo UUID seja aceito para outro arquivo.
